...
class Products extends Component {
  getQuery = (params) => {
    const url = process.env.REACT_APP_API_URL + "/products?" + params;
  ...
  nextLink = (pageInfo) => {
    if (pageInfo.nextPage !== null) {
      return (
        <Link to={{ pathname: "/product", search: pageInfo.nextPage }}>
          {" "}
          Next{" "}
        </Link>
      );
  ...
  prevLink = (pageInfo) => {
    if (pageInfo.prevPage !== null) {
      <Link to={{ pathname: "/product", search: pageInfo.prevPage }}>
        {" "}
        Prev{" "}
      </Link>;
    }
          ...
          <ProductsComp
            ...
            subtitle={
              this.props.history.location.search !== ""
                ...
                : "All products we have here !"
            }
            products={product.products.data && product.products.data.products}
          />
